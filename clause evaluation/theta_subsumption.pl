...
:- use_module('../utils/clause', [skolemize/2, skolemize_ignoring/3, undo_skolemize/2, split_pos_neg/3, hypothesisBody/2, hypothesisHead/2]).

% YAP modules
:- use_module(library(ordsets), [list_to_ord_set/2, ord_del_element/3, ord_intersection/3, ord_subtract/3, ord_union/3]).
:- use_module(library(lists), [append/3, delete/3, member/2, memberchk/2, min_list/2, nth/4, reverse/2, flatten/2]).
...
theta_subsumes_exampleids(Hypothesis, ExampleIDs, SubsumedExampleIDs):-
  (setting(useneg, true) ->
  theta_subsumes_exampleids_neg(Hypothesis, ExampleIDs, inf, _, SubsumedExampleIDs)
   ;
...
%    trace,
  (setting(useneg, true) ->
  theta_subsumes_exampleids_neg([Head|Body], ExampleIDs, MaxWeight2Cover, WeightCovered, SubsumedExampleIDs)
       ;
...
theta_subsumes_neg([ClauseHead | ClauseBody], Example) :-
  split_pos_neg(ClauseBody, Pos, Neg),
  PosClause = [ClauseHead | Pos],
  theta_subsumes(PosClause, Example),
  subsumes_none(ClauseHead, Neg, Example).

subsumes_none(_, [], _).

subsumes_none(Head, [\+H|T], Example) :-
	seq_to_list(H, HL),
	\+theta_subsumes([Head | HL], Example),
	subsumes_none(Head, T, Example).

seq_to_list((H, T), [H|NT]) :-
	!,
	seq_to_list(T, NT).
...
theta_subsumes_exampleids_neg([Head|Body], ExampleIDs, MaxWeight2Cover, WeightCovered, SubsumedExampleIDs):-
  %skolemize([Head|Body], Hyp),
  %portray_clause(Hyp),nl,
  split_pos_neg(Body, Pos, Neg),
  %first the positives
  preprocess_hypothesis([Head|Pos], PPLitsInfo, PVarPredCalls, PUniqPredCalls, PPairwiseVarInters, PFullVarInters),
  PHypDS=hyp_ds(Head, PPLitsInfo, PVarPredCalls, PUniqPredCalls, PPairwiseVarInters, PFullVarInters),
  theta_subsumes_exampleids(ExampleIDs, PHypDS, MaxWeight2Cover, 0, WeightCoveredP, PSubsumedExampleIDs),
  %then on negative
  preprocess_Neg(Head, Neg, NegHypDS),
  fail_all(PSubsumedExampleIDs, NegHypDS, MaxWeight2Cover, WeightCoveredP, WeightCovered, SubsumedExampleIDs).
...
preprocess_Neg(Head, [\+ B | Bodies], [HDS | Hypdss]) :-
    preprocess_Neg(Head, Bodies, Hypdss),
    parenthToList(B, NB),
    Tmp = [Head | [NB]],
    flatten(Tmp, Hyp),
    preprocess_hypothesis(Hyp, PLI, VPC, UPC, PVI, FVI),
    HDS = hyp_ds(Head, PLI, VPC, UPC, PVI, FVI).

fail_all(EIDs, [], MaxWeight2Cover, CurCovered, CurCovered, EIDs).

fail_all(ExampleIDs, [HD | HDS], MaxWeight2Cover, CurCovered, WeightCovered, SubsumedExamples) :-
    fail_all(ExampleIDs, HDS, MaxWeight2Cover, CurCovered, WeightCoveredT, SubsumedExamplesT),
    theta_subsumes_exampleids(SubsumedExamplesT, HD, MaxWeight2Cover, 0, WeightCoveredTT, SubsumedExamplesTT),
    list_to_ord_set(SubsumedExamplesT, SET),
    list_to_ord_set(SubsumedExamplesTT, SETT),
    ord_subtract(SET, SETT, SubsumedExamples),
    WeightCovered is WeightCoveredT - WeightCoveredTT,
    (WeightCovered < 0 -> trace ; true).
